...
_Last updated: 2026-06-11 (Thursday — Headliner "mini mode" shipped: square cover-first popup with hover-reveal listen toggle)_
...
### 2026-06-27 — Search by song

Collection search now matches **track names**, not just artist/title/tag. Motivated by the "I know I own this song, can't recall the album" case; built only after confirming it's viable, since track search is worthless without tracklist coverage (was ~1% — snapshots only ever got written by manual chooser pins).

- **Bulk tracklist backfill** (Settings → **Backfill tracklists**): `src/lib/tracklist-backfill.server.ts`, action `backfillTracklists` in `settings/+page.server.ts`. Pins a snapshot on every album where `tracklist IS null` (OWN + WANT) using the existing `fetchTracklist` pick-longest resolver; concurrency 3 to stay gentle on MusicBrainz. Writes the same `{tracks, source, sourceId?}` shape a manual pin produces, so backfilled and curated snapshots are indistinguishable. Recap shows filled / tracks-indexed / still-empty (with links). Brent's run: 127/127 filled, 0 empty, ~100% coverage.
- **Persist-on-view** (`albums/[id]/+page.server.ts`): the add flow doesn't pin a tracklist, so without this, coverage would erode as albums get added. When the album page live-fetches a tracklist (no snapshot existed), it now pins it — fire-and-forget, best-effort, never blocks the render. Bulk backfill stays the catch-up for never-opened albums.
- **Search surfaces** (both): track hits resolve to **album cards/rows**, never a separate tracks mode. Artist/title/tag rank first; track-only matches follow, each showing the matched song (`↳ "Name"`).
  - Global ⌘K **"Do I have this?"** palette — the primary surface (`LookupPalette.svelte`). `/api/albums/lookup` now ships each album's flattened track names (jsonb stripped off the row).
  - `/u/[username]/collection` page search — bundles a compact `trackIndex` (Record<albumId, names[]>) into the page load. Include-in-payload chosen over server-side jsonb query: ~300 bytes/album gzipped, fine to ~1,500-album collections (flip point is payload, not CPU). Search stays 100% client-side/instant.
- **Scoped out:** wantlist page has no track search (you don't "own" those songs). The ⌘K palette does cover WANT, but that's incidental — it already searched both lists by title.
...
- **Headliner mini mode** at `/headliner/[username]/mini`: a compact, corner-of-the-screen popup for listening alongside other work. **The mini _is_ the album cover** — a square that full-bleeds at a square content area and letterboxes against the same blurred backdrop off-square, so it survives any window shape. Built aspect-agnostic via CSS size containers: `.frame` (`container-type: size`) sizes `.art` to `min(100cqw, 100cqh)` (largest fitting square); `.art` is itself a container so the scrim text scales to the _cover_, not the window. Track info (eyebrow / track / artist) + the owner-only **last-caught line** (flashes accent on a new spin) sit in a bottom gradient scrim over the cover. Two glyph circles float over the cover, sharing a `.corner-btn` base (circle at rest, label hover-reveals): **top-right is the owner-only mic** — state carried by colour + pulse (cream off, red armed, fast-pulse identifying), expanding its `Listen / Listening / Identifying…` label on `:hover`/`:focus-visible`; **top-left is a swap-to-full button** (all viewers) that opens the full Headliner near-fullscreen and closes the mini itself in one gesture. The self-close works because the mini was script-opened (`window.open` is allowed to `close()` its own windows; a user-opened window like the full Headliner can't be closed by script — which is also why full→mini can't auto-close the full). It only self-closes if the full view actually opened (popup-blocker guard). **Mic reuses the global `spin` store + `SpinSessionRunner`** (root layout), so there's no recorder code on the page — it only calls `spin.toggle()` and reads back state. Trimmed copy of the full Headliner's poll/accent/cover-fallback logic, minus the idle-streak + mosaic machinery (no `idleTiles` fetch). Opened from a `⊟ Mini` link by the full Headliner's back-link, which calls `window.open(..., 'popup,width=240,height=240')` so it pops square and small (modified/middle clicks fall through to a normal tab). `suppressPill` already covers the route via `startsWith('/headliner/')`. Documented for users in the **Headliner** section of `/help`. **Known limitation — double-catch:** running the full Headliner and the mini with listen on at the same time records the same spin twice (two independent MediaRecorders against one global store) — the swap-to-full button sidesteps this by closing the mini as it reopens the full view, but nothing stops a user from manually keeping both open + listening. Full cross-window coordination (BroadcastChannel/storage handshake) deliberately deferred since in practice you use one _or_ the other.
...
- **"Do I have this?" lookup palette** — Cmd/Ctrl-K shortcut + search-icon trigger; centered modal on desktop, full-screen sheet on mobile. Matches artist, album title, **or song** (track-only hits show `↳ "Name"`). Empty state offers "+ Add this album" prefill link.
...
- Lookup palette ("Do I have this?"): `src/lib/components/LookupPalette.svelte`, state in `src/lib/lookup-state.svelte.ts`, data endpoint `src/routes/api/albums/lookup/+server.ts` (ships flattened per-album track names alongside artist/title). Mounted globally in `+layout.svelte` for signed-in users; trigger button + Cmd/Ctrl-K shortcut. Matches artist/title first, then track-only hits. Bulk tracklist backfill that feeds it: `src/lib/tracklist-backfill.server.ts`; coverage kept fresh by persist-on-view in `src/routes/albums/[id]/+page.server.ts`
...
- **Multi-disc tracklists renumber on read and write.** Catalog sources restart `track_number` at 1 per disc, so duplicate positions arrive on double albums (and live in pre-fix snapshots). `renumberIfDuplicates()` in `src/lib/tracklist.ts` normalizes both paths; the display each-blocks are deliberately unkeyed.
- **`xlsx` must come from SheetJS's CDN, never the npm registry.** The registry copy is abandoned at 0.18.5 with high-severity advisories; package.json pins `https://cdn.sheetjs.com/xlsx-0.20.3/xlsx-0.20.3.tgz`. A plain `npm install xlsx` would silently reintroduce the vulnerable build.
- **`npm run lint` is green as of 2026-06-10 — keep it green.** One-time prettier pass landed; eslint intentionally disables `svelte/no-navigation-without-resolve`, `svelte/require-each-key` (duplicate keys are the real hazard here — don't re-enable), and `svelte/prefer-svelte-reactivity`, each with rationale comments in `eslint.config.js`. `svelte/no-at-html-tags` stays on with two justified inline disables at the escape-first `notesToHtml` sites.
