...
<div class="page">
	...
	<section class="lede">
		<p>
			...
			kept, revisited, occasionally read aloud to a friend. A collection isn't an inventory.
			It's a slow autobiography you write one purchase at a time.
		</p>
		<p>
	...
	<section id="start" class="card">
		...
		<ol>
			<li>Sign up, confirm your email, and pick a username on the welcome screen — that becomes the address of your public page.</li>
			<li>Add albums with <strong>+ Add</strong> on the home page, or import a CSV / Discogs export from Settings.</li>
			<li>Connect Last.fm in Settings if you scrobble — this unlocks the Headliner stage and Spin.</li>
			<li>Visit your public page at <code>/u/&lt;your-username&gt;</code>. Pick a featured album in Settings to anchor it.</li>
			<li>Open the Headliner for a full-screen "what I'm playing right now" view. Install it as a PWA on a tablet next to the stereo if you want the full effect.</li>
		</ol>
	</section>
	...
	<section id="collection" class="card">
		...
		<p><strong>Pick a tracklist.</strong> The same panel also surfaces a <strong>Tracklist</strong> chooser — one row per source that returned tracks (Spotify, Deezer, iTunes, Last.fm), with track count + total runtime. Click a row to expand the full list; click <strong>Accept</strong> to pin that source's tracklist to the album. The album page reads the pinned snapshot on every load, so what you picked is what you see. <strong>Use auto-pick</strong> clears the pin and goes back to "longest wins."</p>
	...
	<section id="wantlist" class="card">
		<h2>The Wantlist</h2>
		<p>Anything you mark as <em>want</em> lives at <code>/wantlist</code>. The home page shows only what you actually own.</p>
		<p>Each wantlist album has a row of buy-links — Record Exchange first (Boise's local shop), then Discogs and the streaming services. When you finally pick the thing up, click <strong>✓ I got it!</strong> on the album page and it promotes to your collection.</p>
	</section>

	<section id="covers" class="card">
		<h2>Covers &amp; look-up</h2>
		<p><strong>Auto-search.</strong> When you save an album without a cover, Albumz pulls in parallel from Spotify, iTunes, Last.fm, MusicBrainz / Cover Art Archive, and Deezer, scores by how well artist and title match, and uses the top hit — <em>but only when the match is strong enough</em>. Artist names that only partially match (the old "Helium" / "Hellium" / "Helium Sound" trap) no longer auto-attach; ambiguous albums stay in the missing-covers list so you can pick manually.</p>
	...
	<section id="public" class="card">
		...
		<ul>
			<li>A <strong>featured album</strong> — atmospheric hero with the cover, accent color from the art, and a fade into the background.</li>
			...
			<li>A link to your <strong>full collection</strong> at <code>/u/&lt;username&gt;/collection</code> for visitors who want to dig.</li>
		</ul>
		<p>The featured album hero is overridden by your current Last.fm now-playing track. If you're actively scrobbling, visitors see what's spinning rather than your pinned pick.</p>
	</section>
	...
	<section id="discover" class="card">
		...
		<ul>
			<li><strong>◉ spun</strong> — physical playback that Albumz heard through your mic and identified</li>
			<li><strong>⇢ streamed</strong> — a track that was on your Last.fm now-playing while Albumz was watching (you weren't playing it physically, you were streaming it)</li>
		</ul>
		<p>The distinction matters because Spotify already scrobbles your streams to Last.fm; Albumz quietly scrobbles your <em>physical</em> plays the same way, so your Last.fm history finally reflects everything you actually listened to.</p>
	...
	<section id="lastfm" class="card">
		...
		<p>Settings → <strong>Last.fm connection</strong> → Connect. You'll authorize Albumz on Last.fm, get bounced back, and Albumz will:</p>
		<ul>
			<li>Use your Last.fm username for now-playing displays on your public page and Headliner</li>
			<li>Send <code>updateNowPlaying</code> whenever Spin identifies something (so the Headliner stage stays current)</li>
			<li><strong>Scrobble</strong> physical plays — about 60 seconds of confirmed presence on the same track is enough to count</li>
	...
	<section id="look" class="card">
		...
		<ol>
			...
			<li>A generated tile with the first letter of your username and a color derived from the username string</li>
		</ol>
		<p>Upload from Settings → Profile. Remove to fall back to Gravatar / generated.</p>
		<p><strong>Themes</strong> live as pills in Settings — pick light, dark, or auto (follows your OS). The accent color throughout the app comes from whatever album cover is "active" — your featured album, the Headliner's current track, or the album you're viewing — so the palette shifts as you move around.</p>
	</section>
	...
	<section id="shortcuts" class="card">
		...
		<table class="shortcuts">
			...
			<tbody>
				<tr><td><kbd>⌘K</kbd> / <kbd>Ctrl-K</kbd></td><td>Anywhere signed in</td><td>Open the Look-up palette</td></tr>
				<tr><td><kbd>Esc</kbd></td><td>Inside Look-up or pickers</td><td>Close</td></tr>
				<tr><td><kbd>↑</kbd> / <kbd>↓</kbd></td><td>Inside Look-up</td><td>Move through results</td></tr>
	...
	<section id="trouble" class="card">
		...
		<dl>
			...
			<dd>Open the album, click <strong>Pick a cover</strong>, choose manually. If none of the candidates are right, click <strong>Edit details</strong> → 🔎 Look up details and pick a match from there.</dd>
			<dt>The Headliner is stuck on a track that finished an hour ago.</dt>
			<dd>Make sure Last.fm is connected in Settings. If you're streaming and the Headliner is stale, your streaming service may have stopped scrobbling.</dd>
			<dt>Spin won't turn on.</dt>
			<dd>It needs microphone permission and HTTPS. Browser permission prompts are per-origin; clearing site data resets them.</dd>
	...
	<section class="card foot">
		<p>Found something broken? Open an issue at
			<a href="https://github.com/spudalicious1969/albumz/issues" target="_blank" rel="noopener">github.com/spudalicious1969/albumz/issues</a>,
...
<style>
	...
	dl { margin: 0; font-size: 0.95rem; line-height: 1.6; }
	dt { font-weight: 600; color: var(--text); margin-top: 0.85rem; }
	dt:first-child { margin-top: 0; }
	dd { margin: 0.2rem 0 0; color: var(--text-muted); padding-left: 0; }
